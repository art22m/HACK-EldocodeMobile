...
class SaveActions: ISaveActions {
    ...
    func saveProducts(products: [Product], manager: String, customerPhone: String?) {
        ...
        let id = "staffID12345"
        db.collection("carts").addDocument(data: ["client_ref": db.document("/client/\(customerPhone ?? "Незивестен")"), "datetime" : Date.init(), "products" : productsLinkAndName, "staff_ref": db.document("/staff/\(id)")])
    }
